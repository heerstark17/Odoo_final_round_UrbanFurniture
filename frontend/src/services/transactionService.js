import { getContacts, getProducts } from "./masterDataService";
...
export const transactionService = {
  getMasters: async (partyType) => {
    const { contacts, products, analytics } = await getMasterData();

    const parties = contacts.filter((item) => {
      const type = String(
        item.type || item.contactType || ""
      ).toLowerCase();

      return type.includes(partyType);
    });
  ...
  createOrder: async ({
    ...
    const data = read();
    const { contacts, products } = await getMasterData();
    ...
    const party = contacts.find(
      (item) =>
        String(idOf(item)) === String(partyId)
    );
  ...
  confirmOrder: async (type, id) => {
    ...
    const order = data[collection].find(
      (item) => item.id === id
    );
  ...
  createDocument: async (type, orderId) => {
    ...
    const order = data[orderCollection].find(
  ...
  registerPayment: async ({
    ...
    const document = data[collection].find(
    ...
    if (
      payment <= 0 ||
      payment > document.amountDue
    ) {
      throw new Error(
        "Payment must be greater than zero and cannot exceed amount due."
      );
    }
