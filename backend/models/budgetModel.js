...
async function getById(id) {
  return (await pool.query("SELECT * FROM budgets WHERE id = $1", [id]))
    .rows[0];
}
...
async function update(id, data) {
  const result = await pool.query(
    `UPDATE budgets SET budget_name = $1, start_date = $2,
...
async function remove(id) {
  return (
    await pool.query("DELETE FROM budgets WHERE id = $1 RETURNING *", [id])
  ).rows[0];
}
